...
const FormDialog = function (props: FormDialogProps) {
  ...
  const handleSubmit = props.handleSubmitConstructor({
    resetIndexData,
    resetLoadingState,
    setErrors,
    setNotifications,
  ...
  return (
    <div>
      ...
      <Dialog fullWidth open={open} onClose={handleClose}>
        ...
        <DialogContent>
          <DialogContentText>{props.inputText}</DialogContentText>
          {props.delete ? (
...
const StatusNotification = function () {
  ...
  const handleClose = function () {
    ...
    return result === null
      ? console.error("Notification dispatch setter is null")
      : result;
  };
  ...
  return (
    <>
      {notificationKeys === null
        ? console.error("Notifications are null")
        : notificationKeys.map((key) =>
            convertToSnackbar(key as keyof NotificationReducerInterface)
          )}
    </>
  );
};

const { addCommentProps, deleteCommentProps, editCommentProps } =
...
const Comment = function (props: CommentInterface) {
  ...
  const isUserCommentOwner =
    user && props.user && user?._id === props.user?._id;
...
const Post = function Post() {
  ...
  const post =
    postId && posts && posts.length > 0 ? extractPostById(postId, posts) : null;
            ...
            <>
              ...
              {post.comments.length === 0 ? (
                <Typography variant="subtitle2">
                  There don't appear to be any comments yet. Be the first to
                  give your thoughts on this post.
                </Typography>
              ) : (
                false
              )}
                ...
                <Stack spacing={2} sx={{ p: 2 }}>
                  {(user?.member_status === "privileged" ||
                    ...
                    <Grid container spacing={2} wrap="nowrap">
                      ...
                      <Grid item>
                        <FormDialog
                          {...addCommentProps(postId as string, user._id)}
                        />
                      </Grid>
                    </Grid>
                  )}
