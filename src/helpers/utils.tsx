...
export const handleStatus401 = function (res: Response) {
  const unauthorisedResponse = function () {
    const errors = {
      errors: {
        msg: "The submitted username and password combination is not valid. Please try again.",
      },
    };
    const blobErrors = new Blob([JSON.stringify(errors)], {
...
export const parseAPIErrors = function (
  response: Record<"errors", Record<string, string | Record<string, string>>>
) {
...
export const produceDefaultErrorPairs = function (errorKeys: string[]) {
  ...
  return [
    generalError,
    ...errorKeys.map(function convertToKeyValues(key) {
...
export const restoreOriginalErrorState = function (
